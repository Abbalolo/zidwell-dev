...
function generateReceiptHTML(receipt: any, logo: string, signeeName: string) {
  const total = receipt.receipt_items?.reduce((sum: number, item: any) => {
  ...
  <div class="bg-white max-w-4xl mx-auto shadow-lg rounded-lg overflow-hidden">
    ...
    <div class="p-8">
      ...
      <table class="w-full table-auto border mb-6">
        ...
        <tbody>
          ${receipt.receipt_items?.map((i: any) => `
            <tr>
              <td>${i.item}</td>
              <td class="text-center">${i.quantity}</td>
              <td class="text-right">${Number(i.price).toLocaleString("en-NG", { style: "currency", currency: "NGN" })}</td>
              <td class="text-right">${(i.quantity * i.price).toLocaleString("en-NG", { style: "currency", currency: "NGN" })}</td>
            </tr>`).join("") || ''}
        </tbody>
      </table>
      <p class="text-right text-xl font-bold">Total: ${formattedTotal}</p>
      <div class="bg-gray-50 rounded-lg p-4 mt-6">${receipt.customer_note || ""}</div>
      <div class="signatures">
        <div><strong>Initiator:</strong> ${receipt.initiator_name}</div>
        <div><strong>Date:</strong> ${receipt.signed_at || new Date().toLocaleDateString()}</div>
      </div>
...
export async function POST(request: Request) {
  try {
    ...
    const { data: receipt, error } = await supabase
      ...
      .eq("token", token)
      .single();
    if (error || !receipt) return NextResponse.json({ message: "Receipt not found" }, { status: 404});
    ...
    const html = generateReceiptHTML(receipt, logo, receipt.signee_name || "Signee Name");
    ...
    await transporter.sendMail({
      ...
      attachments: [{ filename: `${receipt.receipt_id}.pdf`, content: pdfBuffer }],
    });

    return NextResponse.json({ message: "Receipt signed and emailed" }, { status: 200 });
  } catch (err) {
